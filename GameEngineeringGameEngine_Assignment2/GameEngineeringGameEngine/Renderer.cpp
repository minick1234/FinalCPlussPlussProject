...
void Renderer::Initialize(int _xResolution, int _yResolution) {
	...
	m_window = SDL_CreateWindow("Assignment 2 - Dominik Ziolkowski - Based off the Framework From Class", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, _xResolution, _yResolution, SDL_WINDOW_SHOWN);
...
void Renderer::Shutdown() {
	...
	SDL_Quit(); // Quit The sdl subsystems
}
...
void Renderer::RenderTexture(Texture* _texture, Rect _rect) {
	...
	m_destRect.w = _rect.X2 - _rect.X1;
	m_destRect.h = _rect.Y2 - _rect.Y1;
	M_ASSERT(((SDL_RenderCopyEx(m_renderer, GetSDLTexture(_texture), NULL, &m_destRect, 0, NULL, SDL_FLIP_VERTICAL)) >= 0), "Could not render the texture");
}

void Renderer::RenderTexture(Texture * texture, Rect srcRect, Rect destRect) {
	m_destRect.x = destRect.X1;
	m_destRect.y = destRect.Y1;
	m_destRect.w = destRect.X2 - destRect.X1;
	m_destRect.h = destRect.Y2 - destRect.Y1;

	m_srcRect.x = srcRect.X1;
	m_srcRect.y = texture->GetImageInfo()->Height - srcRect.Y2;
	m_srcRect.w = srcRect.X2 - srcRect.X1;
	m_srcRect.h = srcRect.Y2 - srcRect.Y1;
	M_ASSERT(((SDL_RenderCopyEx(m_renderer, GetSDLTexture(texture), &m_srcRect, &m_destRect, 0, NULL, SDL_FLIP_VERTICAL)) >= 0), "Could not render the texture");

}

SDL_Texture* Renderer::GetSDLTexture(Texture* _texture) {
	//If the texture has already been created, return the created texture;
	...
	//otherwise create the new gpu texture.
...
void Renderer::SetDrawColour(Color _color) {
